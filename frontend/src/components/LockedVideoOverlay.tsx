...
const LockedVideoOverlay = ({ title, onUnlock }: LockedVideoOverlayProps) => {
  ...
  return (
    <div
      ...
      aria-label={t("LockedVideoOverlay.conteudoBloqueado", { title })}
    >
      ...
      <p className="relative text-sm font-medium uppercase tracking-widest text-primary-foreground/75 mb-1">
        {t("LockedVideoOverlay.conteudoPremium")}
      </p>
      <p className="relative text-base font-semibold mb-4 max-w-xs">
        {t("LockedVideoOverlay.facaUpgradeParaDesbloquear")}
      </p>
      <Button
        size="sm"
        onClick={handleClick}
      ...
      >
        ...
        {t("LockedVideoOverlay.desbloquear")}
      </Button>
    </div>
  );
};
