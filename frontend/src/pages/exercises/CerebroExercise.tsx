...
// Hit-box bem mais indulgente que os outros exercícios de propósito: os
// alvos aqui vivem na periferia da grelha, onde o rastreio ocular por
// webcam é menos preciso -- ver TrackingExercise.tsx para a mecânica base.
...
// Grelha de formas (a "logica visual actual" do exercício): 16 posições
// fixas, uma delas destacada de cada vez como alvo activo. O utilizador tem
// de encontrar e fixar o olhar nessa forma entre as restantes (distractores).
const GRID_COLS = 4;
const GRID_ROWS = 4;
const GRID_SIZE = GRID_COLS * GRID_ROWS;
const CELULA_DIAMETRO_PX = 56;
// Ritmo a que o alvo activo muda de posição, à velocidade "Normal" -- igual
// a COMBO_TIMEOUT_MS de propósito, para que um alvo nunca desapareça antes
// de o combo ainda poder ser salvo por um Hit a tempo.
const ROTACAO_ALVO_MS_BASE = COMBO_TIMEOUT_MS;
// Pulsação visual (brilho/escala) do alvo activo -- puramente decorativa,
// não faz parte da mecânica de acerto. Ângulo contínuo, nunca reiniciado
// por módulo (mesma lição aprendida no Acompanhamento em Oito).
const PULSO_DURACAO_MS = 700;
const VELOCIDADE_ANGULAR_PULSO = (Math.PI * 2) / (PULSO_DURACAO_MS / 1000);

// Área útil da grelha: limitada e centrada horizontalmente (equivalente a
// envolver a grelha num `max-w-md mx-auto`) para que nenhum alvo fique
// perto das bordas do ecrã, onde o rastreio ocular por webcam falha mais.
...
/** Escolhe um novo índice de grelha diferente do actual. */
...
// Mesma lógica dos outros exercícios, copiada para manter consistência.
 ...
 */
const CerebroGame = ({ duracaoSegundos, onEscolherDuracao }: CerebroGameProps) => {
  const { locked } = useExerciseSession();
  if (locked) return <div className="h-[350px]" />;
  return (
    <CerebroGameAtivo duracaoSegundos={duracaoSegundos} onEscolherDuracao={onEscolherDuracao} />
  );
};
...
const CerebroGameAtivo = ({ duracaoSegundos, onEscolherDuracao }: CerebroGameProps) => {
  ...
  // Roda o alvo activo pela grelha, pulsa-o visualmente e, no mesmo frame,
  // verifica o cruzamento discreto do olhar com ele (Hits + combo).
  useEffect(() => {
    ...
    const animar = (timestamp: number) => {
      ...
      // Pulsação visual do alvo activo (0-1), só decorativa.
      const anguloPulso = elapsedSeconds * VELOCIDADE_ANGULAR_PULSO;
      setPulso(0.5 + 0.5 * Math.sin(anguloPulso));

      // Rotação do alvo activo, ao ritmo escolhido em "Velocidade".
      ...
      // Cruzamento válido: olhar dentro do hit-box do alvo activo E fora
      // do cooldown desde o último Hit -- evita o efeito metralhadora de
      ...
      if (
        ...
        // Recompensa imediata: avança logo para um novo alvo em vez de
        // esperar pelo resto do intervalo de rotação.
        rodarAlvo();
      }
  ...
  // discretos acima: esta é uma média contínua, não um evento por cruzamento.
  useEffect(() => {
    if (!isRunning) return;
  ...
  // Pulso reforçado de propósito -- os alvos vivem na periferia da
  // grelha, por isso o destaque tem de chamar a atenção da visão
  // periférica mais depressa do que um "leve" pulso central chamaria.
    ...
    <div className="p-5">
      ...
      >
        ...
        {isRunning && containerSize.width > 0 && (
          <>
            {/* Grelha de formas -- distractores estáticos + o alvo activo,
                destacado e a pulsar. Sem classes de transição/animação
                Tailwind aqui de propósito: o pulso é 100% controlado pelo
                requestAnimationFrame acima, para nunca "lutar" com CSS. */}
              ...
              return (
                <div
                  ...
                  className={`absolute rounded-full border-2 ${
                    ehActivo ? "border-teal bg-teal" : "border-border bg-muted/60"
                  }`}
                  style={{
                    ...
                    boxShadow: ehActivo
                      ? `0 0 ${brilhoRaioPx}px rgba(45, 212, 191, ${brilhoAlfa})`
                      : undefined,
                  }}
                />
              );
...
export default CerebroExercise;
