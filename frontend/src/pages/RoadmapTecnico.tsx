...
const AUDIT: AuditRow[] = [
  ...
  { pagina: "Homepage (/)", componente: "TeamSection", estado: "Mock local (memória: estático propositado)", plataformas: "Manter estático OU Supabase (tabela team_members)", plano: "Manter estático conforme decisão do projecto; se dinâmico, criar tabela + bucket avatares." },
  ...
  // Auth
  { pagina: "Auth (/auth, /login, /registo)", componente: "Login/Signup email+password", estado: "Funcional (Supabase Auth)", plataformas: "Supabase Auth", plano: "Activar HIBP password check, definir política de força mínima, adicionar verificação de email obrigatória." },
  ...
  { pagina: "Exercícios (/exercicios/tracking)", componente: "Tracking ocular em oito", estado: "Animação visual, sem tracking real", plataformas: "MediaPipe Iris + WebGazer.js", plano: "Activar webcam, correr MediaPipe Iris, comparar gaze vector com trajectória alvo, calcular score de precisão, guardar sessão em exercise_sessions." },
  { pagina: "Exercícios (/exercicios/convergencia|cerebro|relaxamento)", componente: "Exercícios interactivos", estado: "Funcional (UI), sem persistência", plataformas: "Supabase (exercise_sessions)", plano: "Criar tabela exercise_sessions (user_id, exercise_slug, score, duration_s, completed_at) e gravar ao concluir." },
  ...
  // Parceiros
  { pagina: "Parceiros (/parceiros)", componente: "Benefícios + CTA (RBAC activo)", estado: "UI condicional funcional", plataformas: "Supabase (tabela partners)", plano: "Criar tabela partners + fluxo de candidatura; ligar 'Quero ser parceiro' a formulário real gravado em premium_requests ou partner_applications." },
  ...
  // Produto / Óculos
  { pagina: "Produto (/produto)", componente: "Catálogo de óculos + carrinho", estado: "MOCK: sem carrinho real, sem checkout", plataformas: "Supabase (products, orders) + Proxypay/EMIS Multicaixa Express (Angola) OU Stripe", plano: "1) Criar tabelas products, cart_items, orders, order_items. 2) Integrar gateway angolano (Proxypay) via edge function server-to-server. 3) Emitir referência multicaixa e webhook de confirmação. 4) Enviar factura via Resend." },
  ...
  // Registo Premium
  { pagina: "Registo Premium (/registo-premium)", componente: "Formulário de subscrição pro", estado: "Grava em premium_requests via edge function (assumido)", plataformas: "Supabase + Stripe Billing/Paddle", plano: "Substituir por checkout recorrente real (Stripe Subscriptions); webhook actualiza role para 'profissional' em user_roles." },
  ...
  { pagina: "Admin (/admin/utilizadores)", componente: "Gestão de utilizadores", estado: "Funcional parcialmente", plataformas: "Supabase Admin API (service_role via edge function)", plano: "Acções destrutivas (ban/delete) só via edge function com verificação has_role('admin')." },
  { pagina: "Admin (/admin/mensagens)", componente: "Inbox contact_messages", estado: "Funcional", plataformas: "Supabase", plano: "Adicionar estado (novo/lido/respondido) e resposta directa via Resend." },
  ...
  // Global
  { pagina: "Global", componente: "GlobalBanner", estado: "Lê tabela banners", plataformas: "Supabase Realtime", plano: "Activar subscrição realtime para banners aparecerem sem refresh." },
...
const RoadmapTecnico = () => {
  const csv = useMemo(() => {
    const header = ["Página/Aba", "Componente/Funcionalidade", "Estado Actual", "Plataformas Recomendadas", "Plano de Acção"];
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-3 text-muted-foreground max-w-3xl text-justify">
              Mapa exaustivo do estado actual (UI vs. funcional) de todas as páginas, rotas e componentes,
              com as plataformas recomendadas e o plano de acção para tornar cada funcionalidade 100% real
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/60 text-left">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold min-w-[320px]">Plano de Acção</th>
                </tr>
              </thead>
              <tbody>
                {AUDIT.map((r, i) => {
